...
k3.metric("💰 Costos Totales", f"USD {abbreviate_number(df['COSTOS DE GENERACIÓN USD'].sum())}")
k4.metric("⚡ Valor prom. KW", f"USD {abbreviate_number(df['VALOR POR KW GENERADO'].mean())}")
...
f1.metric("🔋 Generación", abbreviate_number(df_f["TOTAL GENERADO KW-H"].sum()))
f2.metric("⛽ Consumo", abbreviate_number(df_f["CONSUMO (GLS)"].sum()))
f3.metric("💰 Costos", f"USD {abbreviate_number(df_f['COSTOS DE GENERACIÓN USD'].sum())}")
f4.metric("⚡ Valor prom. KW", f"USD {abbreviate_number(df_f['VALOR POR KW GENERADO'].mean())}")
...
# ======================================================
st.markdown("---")
st.markdown("## 📈 Generación y Consumo Diario")
...
fig_gen = px.bar(
    ...
    text=gen_loc["TOTAL GENERADO KW-H"].apply(abbreviate_number),
...
fig_gen_line.update_traces(textposition="top center")
